...
public:
	...
	foreach(line; lineBuffers)
	{
		...
		if(tmpSplites.length == 4)
		    parseMiddleware(tmpSplites[3], tmpRoute.middleWareBefore, tmpRoute.middleWareAfter);
...
private:
	...
	if(_prefix)
	{
	    ...
	    output ~= spritArr[0]~"."~spritArr[1]~"."~to!string(spritArr[2].asCapitalized)~_controllerPrefix~"."~spritArr[3];
	}
	else
	{
	    assert(spritArr.length == 3, "Wrong controller config!");
	    output ~= "application"~"."~spritArr[0]~"."~to!string(spritArr[1].asCapitalized)~_controllerPrefix~"."~spritArr[2];
	}
