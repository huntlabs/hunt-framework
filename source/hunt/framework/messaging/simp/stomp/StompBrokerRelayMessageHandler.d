...
public class StompBrokerRelayMessageHandler extends AbstractBrokerMessageHandler {
	...
	protected void handleMessageInternal(MessageBase message) {
		...
		if (StompCommand.CONNECT.equals(command)) {
			...
			this.stats.incrementConnectCount();
			Assert.state(this.tcpClient !is null, "No TCP client available");
	...
	private class StompConnectionHandler implements TcpConnectionHandler!(byte[]) {
		...
		public ListenableFuture!(Void) forward(final MessageBase message, final StompHeaderAccessor accessor) {
	...
	private class SystemStompConnectionHandler extends StompConnectionHandler {
		...
		override
		public ListenableFuture!(Void) forward(MessageBase message, StompHeaderAccessor accessor) {
			try {
