 ...
 * @since 4.0
 */
public abstract class AbstractMessageChannel(T) : MessageChannel, InterceptableChannel { // , BeanNameAware
	...
	override
	public  removeInterceptor(ChannelInterceptor interceptor) {
	...
	override
	public final  send(Message!(T) message) {
		return send(message, INDEFINITE_TIMEOUT);
	}

	override
	public final  send(Message!(T) message, long timeout) {
	...
	protected class ChannelInterceptorChain {
		...
		public Message!(T) applyPreSend(Message!(T) message, MessageChannel channel) {
			Message!(T) messageToUse = message;
		...
		public void applyPostSend(Message!(T) message, MessageChannel channel,  sent) {
			for (ChannelInterceptor interceptor : interceptors) {
				interceptor.postSend(message, channel, sent);
			}
		}

		public void triggerAfterSendCompletion(Message!(T) message, MessageChannel channel,
				 sent, Exception ex) {
		...
		public  applyPreReceive(MessageChannel channel) {
			for (ChannelInterceptor interceptor : interceptors) {
		...
		public Message!(T) applyPostReceive(Message!(T) message, MessageChannel channel) {
			Message!(T) messageToUse = message;
			for (ChannelInterceptor interceptor : interceptors) {
